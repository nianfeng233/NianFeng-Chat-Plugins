    ...
    return section(
      ...
      card(
        row(
          ...
          row('旧事件补发上限', '休眠 / 重装 / 重启 / GitHub PushEvent 延迟后，只补发这个时间以内的新事件。默认 30 分钟；遇到 GitHub 延迟建议调到 6 或 24 小时。', select('channels.maxEventAgeMs', Number(channelsConfig.maxEventAgeMs) || 1800000, [5, 10, 30, 60, 180, 360, 720, 1440].map(value => ({ value: value * 60000, label: value < 60 ? `${value} 分钟` : value < 1440 ? `${value / 60} 小时` : '24 小时' })), { width: 130 })) +
          row('通知队列过期时间', '通知生成后超过这个时间仍未投递，就自动作废。默认 30 分钟；和补发上限一起调大，避免延迟事件刚生成就过期。', select('channels.maxPendingAgeMs', Number(channelsConfig.maxPendingAgeMs) || 1800000, [5, 10, 30, 60, 180, 360, 720, 1440].map(value => ({ value: value * 60000, label: value < 60 ? `${value} 分钟` : value < 1440 ? `${value / 60} 小时` : '24 小时' })), { width: 130 })) +
